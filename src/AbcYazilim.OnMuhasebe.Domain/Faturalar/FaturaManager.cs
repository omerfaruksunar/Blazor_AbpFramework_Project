...
public class FaturaManager : DomainService
{
	...
	public FaturaManager(IFaturaRepository faturaRepository,
		ICariRepository cariRepository, IOzelKodRepository ozelKodRepository,
		...
		Guid? ozelKod2Id, Guid? subeId, Guid? donemId)
	{
		...
		await _faturaRepository.KodAnyAsync(faturaNo, x => x.FaturaNo == faturaNo &&
	...
	public async Task CheckUpdateAsync(Guid id, string faturaNo, Fatura entity,
		Guid? cariId,Guid? ozelKod1Id, Guid? ozelKod2Id)
	{
		await _faturaRepository.KodAnyAsync(faturaNo, x => x.Id != id && x.FaturaNo == faturaNo &&
			x.SubeId == entity.SubeId && x.DonemId == entity.DonemId,
