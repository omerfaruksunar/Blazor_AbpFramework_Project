...
public class BankaSubeManager : DomainService
{
	...
	public async Task CheckCreateAsync(string kod, Guid? bankaId, Guid? ozelKod1Id,
		Guid? ozelKod2Id)
	{
		//Bankanın db'de olup olmadigini kontrol ediyoruz.
		await _bankaRepository.EntityAnyAsync
			(bankaId, x => x.Id == bankaId);
		await _bankaSubeRepository.KodAnyAsync
			(kod, x => x.Kod == kod && x.BankaId == bankaId);
		await _ozelKodRepository.EntityAnyAsync
			(ozelKod1Id, OzelKodTuru.OzelKod1, KartTuru.BankaSube);
		await _ozelKodRepository.EntityAnyAsync
			(ozelKod2Id, OzelKodTuru.OzelKod2, KartTuru.BankaSube);
	}
	public async Task CheckUpdateAsync(Guid id, string kod, BankaSube entity,
		Guid? ozelKod1Id, Guid? ozelKod2Id)
	{
		await _bankaSubeRepository.KodAnyAsync(kod, x => x.Id != id &&
			x.Kod == kod && x.BankaId == entity.BankaId, entity.Kod != kod);
