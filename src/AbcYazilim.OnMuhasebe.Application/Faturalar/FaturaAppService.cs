...
public class FaturaAppService : OnMuhasebeAppService, IFaturaAppService
{
	...
	public FaturaAppService(IFaturaRepository faturaRepository, 
		FaturaManager faturaManager, FaturaHareketManager faturaHareketManager)
	{
		...
		(FaturaListParameterDto input)
	{
		...
		var totalCount = await _faturaRepository.CountAsync(x => x.FaturaTuru == input.FaturaTuru && x.SubeId == input.SubeId &&
				 x.DonemId == input.DonemId && x.Durum == input.Durum);
	...
	public virtual async Task<SelectFaturaDto> UpdateAsync(Guid id, UpdateFaturaDto input)
	{
		...
		{
			await _faturaHareketManager.CheckUpdateAsync(faturaHareketDto.StokId, faturaHareketDto.HizmetId,
				faturaHareketDto.MasrafId, faturaHareketDto.DepoId);
